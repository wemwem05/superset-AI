...
```bash
AGENT_SKILLS_DEBUGGER=$(node "/Users/alimuratbekov/work/superset/.claude/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-debugger 2>/dev/null)
```
...
```bash
node "/Users/alimuratbekov/work/superset/.claude/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase_num}): add root causes from diagnosis" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
